...
import { CrossDetector, CrossSignal } from './crossDetector';
...
export class BinanceService {
  ...
  private interval: number = 2000; // 2-second intervals
  private currentPrice: number = 0;

  constructor(symbol: string, shortPeriod: number = 5, longPeriod: number = 15) {
    this.symbol = symbol;
      ...
      trades.forEach((trade: any) => {
        ...
        if (!candleMap.has(timestamp)) {
          ...
          if (mas.shortMA !== null && mas.longMA !== null) {
            ...
            // Check for crosses
            const crossSignal = this.crossDetector.update(mas.shortMA, mas.longMA, timestamp);
            if (crossSignal) {
  ...
  async initializeMovingAverages(): Promise<void> {
    try {
      ...
      for (let i = 0; i < 200; i++) { // 200 is the longest MA period
  ...
  private updateMovingAverages(price: number, timestamp: number): MovingAverages {
  ...
  subscribeToTrades(
    ...
    const intervalId = setInterval(() => {
      if (this.currentCandle) {
        ...
        const mas = this.updateMovingAverages(this.currentCandle.close, this.currentCandle.time as number);
        onMAUpdate(mas);

        const crossSignal = this.crossDetector.update(mas.shortMA, mas.longMA, this.currentCandle.time as number);
        if (crossSignal) {
  ...
  async fetchInitialTickerData() {
    try {
      const response = await fetch(`https://api.binance.com/api/v3/ticker/24hr?symbol=${this.symbol}`);
      const data = await response.json();
  ...
  subscribeToTicker(onTickerUpdate: (price: string, priceChange: string, volume: string) => void): () => void {
    const ws = new WebSocket(`wss://stream.binance.com:9443/ws/${this.symbol.toLowerCase()}@ticker`);
