...
import { createNexusClient, NexusClient, createBicoPaymasterClient, Module } from '@biconomy/sdk'
...
export default function Passkey() {
    ...
    const [passkeyValidator, setPasskeyValidator] = useState<Module | null>(null);
            ...
            try {
                ...
                if (isDeployed) {
                    ...
                    if (cachedWebAuthnKey) {
                        ...
                        setPasskeyValidator(passkeyValidator as Module);
    ...
    useEffect(() => {
        async function checkIsDeployed() {
            const isDeployed = await nexusClient?.account.isDeployed();
    ...
    const sendUserOpWithPasskeyValidator = async () => {
        ...
        try {
            const nexusClientWithPasskeyValidator = await createNexusClient({
                ...
                transport: http(),
                module: passkeyValidator as Module,
                bundlerTransport: http(bundlerUrl),
            });
            const hash = await nexusClientWithPasskeyValidator.sendTransaction({
                calls: [
                    {
